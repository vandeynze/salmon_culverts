...
setwd(wd)
setwd("./output")
write_csv(df_pnshp, "PNSHP_full_working.csv")
write_csv(df_prj, "PNSHP_prj_working.csv")

# # Sample projects to investigate data typology ====
# # Pull specific project
# project_pull <- "BPA_1995-057-02_63492"
# df_pnshp %>% filter(project_id == sample(project_pull, 1)) %>% view
# 
...
# 
# # Single worksites (n = 24,135)
# project_pull <-
#   df_pnshp %>% filter(n_worksites == 1) %>%
#   # single action (n = 18,080)
#   # filter(n_action_wrk == 1) %>%
#     # and no metric (n = 13,829)
#     # filter(n_metric == 0) %>%
#     # and one metric (n = 3,317)
#     # filter(n_metric == 1) %>%
#     # and multiple metrics (n = 934)
#     # filter(n_metric > 1) %>%
#   # multi action (n = 6,055)
#   # filter(n_action_wrk > 1) %>%
#   pull(project_id) %>% unique
# df_pnshp %>% filter(project_id == sample(project_pull, 1)) %>% view
# 
# # Multi worksite (n = 5,784)
# project_pull <-
#   df_pnshp %>%
#   filter(n_worksites > 1) %>%
#   # single action type at project level (n = 3,101)
#   filter(n_action_prj_dist == 1) %>%
#     # and no metric (n = 2,520)
#     # filter(n_metric_prj == 0) %>%
#     # and some metric (n = 581)
#     filter(n_metric_prj != 0) %>%
#       # and complete metrics (n = 387)
#       filter(missing_metric == 0) %>%
#         # and multiple metrics (n = 73)
#         filter(n_metric_prj_dist > 1) %>%
#   # multi action type at project level (n = 2,683)
#   # filter(n_action_prj_dist > 1) %>%
#     # and single action at each worksite (n = 168)
#     # filter(n_worksites == n_action_prj) %>%
#     # and multiple action at least at one worksite (n = 2,515)
#     # filter(n_worksites < n_action_prj) %>%
#       # and same actions at all worksites (n = 1,912)
#       # filter(same_action == TRUE) %>%
#       # and different actions at least at one worksite (n = 603)
#       # filter(same_action == FALSE) %>%
#   pull(project_id) %>% unique
# df_pnshp %>% filter(project_id == sample(project_pull, 1)) %>% view
# 
# # Calculate maximum and mean distance in meters between worksites ====
# df_pnshp <-
#   df_pnshp %>%
#   mutate(
#     dist_max = map_dbl(
...
#       ~ max(
#         distm(
#           df_pnshp %>% filter(project_id == .x) %>% select(longitude, latitude)
#         )
#       )
#     ),
#     dist_mean = map_dbl(
...
#       ~ mean(
#         distm(
#           df_pnshp %>% filter(project_id == .x) %>% distinct(worksite_id, .keep_all = TRUE) %>% select(longitude, latitude)
#         )
#       )
#     )
#   )
