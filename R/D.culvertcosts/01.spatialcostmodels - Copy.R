#' ---
#' title: "Culvert Cost Models with Spatially Explicit Data [Updates to address technical review]"
#' author: "B. Van Deynze"
...
df_corr <-
  ...
  select(
    ...
    # blm_1km_buff,
    # pvall_1km_buff,
    `Private land, managed by industry` = pvi_1km_buff,
    `Private land, managed by non-industrial owner` = pvn_1km_buff,
    `Private land, individual or company` = pv_1km_buff,
    `Construction equipment suppliers` = const_totp,
    `Brick, concrete, and related materials suppliers` = brick_totp,
    `Metal materials suppliers` = metal_totp,
    `Sand and gravel sales yards` = sales_coun,
...
(df_margeff_land <- map_df(mods, ~ coeftest(., vcovHC(., "HC1")) %>% tidy(conf.int = TRUE), .id = "model") %>%
...
# Road features
(df_margeff_roads<-map_df(mods, ~ coeftest(., vcovHC(., "HC1")) %>% tidy(conf.int = TRUE), .id = "model") %>%
...
# ____ Unit of observation ----

# ______ Distribution of sites per project

# Histogram

ggplot(data = df_culv) +
  geom_histogram(aes(x = n_worksites), binwidth = 1) +
  ggtitle("Histogram of number of worksites at project level, i = worksite (n = 1,249)") +
  scale_x_continuous(breaks = c(1:10))

ggplot(data = df_culv %>% group_by(project_id) %>% summarise(n_worksites = mean(n_worksites))) +
  geom_histogram(aes(x = n_worksites), binwidth = 1) +
  ggtitle("Histogram of number of worksites at project level, i = project (n = 1,040)") +
  scale_x_continuous(breaks = c(1:10))

tabyl(df_culv$n_worksites)
df_culv %>% group_by(project_id) %>% summarise(n_worksites = mean(n_worksites)) %>% tabyl(n_worksites)

# ______ Results with averaging across worksites

# Calculate project-level data
...
df_proj <-
  ...
  group_by(project_id) %>%
  summarise(
    n_worksites = mean(n_worksites),
    tot_dist = mean(tot_dist),
    action_fishpass_culvrem_prj = mean(action_fishpass_culvrem_prj),
    action_fishpass_culvinst_prj = mean(action_fishpass_culvinst_prj),
    slope = mean(slope),
    bankfull_width = mean(bankfull_width),
    here_paved = calculate_mode(factor(here_paved)),
    here_speed = calculate_mode(factor(here_speed)),
    cat_basin_slope = mean(cat_basin_slope),
    cat_elev_mean = mean(cat_elev_mean),
    nlcd_current_class = calculate_mode(factor(nlcd_current_class)),
    hdens_cat = mean(hdens_cat),
    emp_const = mean(emp_const),
    emp_agforest = mean(emp_agforest),
    ua_dist = mean(ua_dist),
    const_totp = mean(const_totp),
    brick_totp = mean(brick_totp),
    metal_totp = mean(metal_totp),
    sales_coun = mean(sales_coun),
    pv_1km_buff = mean(pv_1km_buff),
    pvi_1km_buff = mean(pvi_1km_buff),
    pvn_1km_buff = mean(pvn_1km_buff),
    basin = calculate_mode(basin),
    project_year = calculate_mode(factor(project_year)),
    project_source = calculate_mode(factor(project_source)),
    cost_per_culvert = mean(cost_per_culvert)
  )


# Estimate project-level model
mod_proj <- update(mod_full, data = df_proj)

summary(mod_proj)

# ____ Metal suppliers ----

# Show correlation plot with VIFs
fig_corrplot

# Show estimates without metal suppliers

mod_nometal <- update(mod_full, . ~ . - metal_totp)

summary(mod_nometal)

mod_nometal_proj <- update(mod_proj, . ~ . - metal_totp)

summary(mod_nometal_proj)

# ____ Tables ----
mods <- list(mod_full, mod_proj, mod_nometal, mod_nometal_proj)
# mods <- mods[["mod_full"]]
mods_pars <-
  map_df(mods, ~coeftest(., vcov. = vcovCL(., cluster = ~ project_id, "HC3")) %>% tidy, .id = "model") %>%
...
mods_pars %>%
  ...
  pivot_longer(-model, names_to = "Term", values_to = "Value") %>%
  pivot_wider(id_cols = Term, names_from = model, values_from = Value) %>%
  rename(Term = 1, "Base" = 2, "Project-level" = 3, "No metal" = 4, "Project-level + No metal" = 5) %>%
  # kable() %>%
  kable(
    caption = "Cost models",
    escape = FALSE,
          ...
          group =
            case_when(
              ...
              factor %in% c("Brick, concrete, and related materials suppliers",
                            "Construction equipment suppliers",
                            "Sand and gravel sales yards") ~ "Economic",
