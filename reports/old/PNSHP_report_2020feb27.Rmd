...
5. The bulk of the land acquisition spending is in the Puget Sound. Habitat Work Schedule and WA RCO (including SRFBD projects) report the most spending on these projects, and both operate mainly in the Puget Sound region. On the other hand, OWRI does not report land acquisition projects, so we may be getting an incomplete picture of acquisition activity in Oregon. Land acquistions are clearly the most expensive actions, so including them together with other actions might obscure some signal.
6. Expenditures have been rising overall and for almost all major actions over time, with the exception of road drainage improvements (see note in the Action and Project Counts section above). Expenditures per project and per action have been rising over time as well. This may be because agencies are attempting more complex projects and extensive actions, or because the mix of actions is shifting towards more expensive actions. It may also be that the "low hanging fruit" is all gone and that there simply aren't more actions available. Of course, it could also be that costs are just rising over time (though I have already adjusted for inflation). As seen in the Counts figures, there is a noticeable drop in spending in the year of the financial crisis, though overall spending quickly recovers.
7. OWRI, which reports the bulk of the Oregon projects, consistently reports very low per project and per action costs. This may be because they do not report land acquisitions, which are the most expensive actions. OWRI also reports a disproportionate number of road drainage projects, which might drive down their costs. They also may just be computing their costs differently than other groups. But this is a red flag for comparing OWRI-reported projects to others.
8. On a per action basis, projects involving channel modification have the second highest costs after land acquisition. Notabely, these projects are more expnsive on a per action basis than other instream actions like weirs, large woody debris, boulders, etc.

### Expenditure Figures {.tabset .tabset-fade .tabset-pills}
*To view high-resolution figure, right click and select "Open image in new tab".*

For figures that break down expenditures by action type, note that these are expenditures on ***projects*** that include ***at least one instance*** of that action type. That is, expenditures per action under the *INSTREAM - LARGE WOODY DEBRIS* action type would be expneditures per action for *projects* that include that action type as opposed to expenditures per action of that action type. For projects with multiple action types, we cannot distinguish between spending on each action type.
...
# Spending per project, by action ====
df_fig_spendproj_action <-
  df_prj %>%
  ungroup() %>%
  mutate_at(
    vars(starts_with("action_")),
...
# Spending per project, by basin ====
df_fig_spendproj_basin <-
  df_prj %>%
  group_by(basin) %>%
  summarize(adj_cost = sum(adj_cost, na.rm = TRUE)/ sum(!is.na(adj_cost))) %>%
  drop_na()
...
# Spending per project, by source ====
df_fig_spendproj_source <-
  df_prj %>%
  group_by(project_source) %>%
  summarize(adj_cost = sum(adj_cost, na.rm = TRUE) / sum(!is.na(adj_cost))) %>%
  drop_na()
...
# Spending per project, by year and action ====
df_fig_spendproj_year_action <-
  df_prj %>%
  ungroup() %>%
  mutate_at(
    vars(starts_with("action_")),
...
# Spending per project, by year and basin ====
df_fig_spendproj_year_basin <-
  df_prj %>%
  group_by(project_year, basin) %>%
  summarize(adj_cost = sum(adj_cost, na.rm = TRUE) / sum(!is.na(adj_cost))) %>%
  drop_na() %>%
...
# Spending per project, by year and source ====
df_fig_spendproj_year_source <-
  df_prj %>%
  group_by(project_year, project_source) %>%
  summarize(adj_cost = sum(adj_cost, na.rm = TRUE) / sum(!is.na(adj_cost))) %>%
  drop_na() %>%
...
# Spending per project, by basin and action ====
df_fig_spendproj_basin_action <-
  df_prj %>%
  ungroup() %>%
  mutate_at(
    vars(starts_with("action_")),
...
# Spending per project, by source and action ====
df_fig_spendproj_source_action <-
  df_prj %>%
  ungroup() %>%
  mutate_at(
    vars(starts_with("action_")),
...
```{r spending_act1_prep, include = FALSE}
# Spending per action, by action ====
df_fig_spendact_action <-
  df_prj %>%
  ungroup() %>%
  mutate_at(
    vars(starts_with("action_")),
...
df_fig_spendact_action <-
  ...
  left_join(df_fig_spendact_action) %>%
  group_by(action) %>%
  summarize(
    adj_cost = sum(adj_cost_act) / sum(n_action_act)
  ) %>%
  mutate(action_full = recode(action, !!!invert(action_key)))
```
```{r spending_act1, fig.dim = c(12, 10)}
fig_spendact_action <-
  ggplot(
    df_fig_spendact_action %>%
      filter(action_full %in% action_levels[1:50]),
...
# Spending per action, by basin ====
df_fig_spendact_basin <-
  df_prj %>%
  group_by(basin) %>%
  summarize(adj_cost = sum(adj_cost, na.rm = TRUE)/ sum(n_action_prj)) %>%
  drop_na()
...
# Spending per action, by source ====
df_fig_spendact_source <-
  df_prj %>%
  group_by(project_source) %>%
  summarize(adj_cost = sum(adj_cost, na.rm = TRUE) / sum(n_action_prj)) %>%
  drop_na()
...
```{r spending_act5_prep, include = FALSE}
# Spending per action, by year and action ====
df_fig_spendact_year_action <-
  df_prj %>%
  ungroup() %>%
  mutate_at(
    vars(starts_with("action_")),
...
df_fig_spendact_year_action <-
  ...
  left_join(df_fig_spendact_year_action) %>%
  group_by(action, project_year) %>%
  summarize(
    adj_cost = sum(adj_cost_act) / sum(n_action_act)
  ) %>%
  mutate(action_full = recode(action, !!!invert(action_key))) %>%
  ...
  drop_na()
```
```{r spending_act5, fig.dim = c(12, 20)}
...
# Spending per action, by year and basin ====
df_fig_spendact_year_basin <-
  df_prj %>%
  group_by(project_year, basin) %>%
  summarize(adj_cost = sum(adj_cost, na.rm = TRUE) / sum(n_action_prj)) %>%
  drop_na() %>%
...
# Spending per action, by year and source ====
df_fig_spendact_year_source <-
  df_prj %>%
  group_by(project_year, project_source) %>%
  summarize(adj_cost = sum(adj_cost, na.rm = TRUE) / sum(n_action_prj)) %>%
  drop_na() %>%
...
```{r spending_act8_prep, include = FALSE}
# Spending per action, by basin and action ====
df_fig_spendact_basin_action <-
  df_prj %>%
  ungroup() %>%
  mutate_at(
    vars(starts_with("action_")),
...
df_fig_spendact_basin_action <-
  ...
  left_join(df_fig_spendact_basin_action) %>%
  group_by(action, basin) %>%
  summarize(
    adj_cost = sum(adj_cost_act) / sum(n_action_act)
  ) %>%
  ...
  drop_na()
```
```{r spending_act8, fig.dim = c(12, 20)}
...
```{r spending_act9_prep, include = FALSE}
# Spending per action, by source and action ====
df_fig_spendact_source_action <-
  df_prj %>%
  ungroup() %>%
  mutate_at(
    vars(starts_with("action_")),
...
df_fig_spendact_source_action <-
  ...
  left_join(df_fig_spendact_source_action) %>%
  group_by(action, project_source) %>%
  summarize(
    adj_cost = sum(adj_cost_act) / sum(n_action_act)
  ) %>%
  ...
  drop_na()
```
```{r spending_act9, fig.dim = c(12, 20)}
