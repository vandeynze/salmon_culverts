  ...
  function(
    ...
    model %>%
      ggpredict(
        ...
        condition = 
          c(here_speed = 6, project_year = 2005)
      ) %>%
      tibble %>%
      mutate(
...
fig_bankfullslope <- (
  map_df(mods, predict_cost_interaction,
         lims1 = c(0, 0.20),
         lims2 = c(0, 20),
         .id = "model") %>% 
  ...
  ggplot() +
    ...
    geom_contour_filled(
      breaks = c(0, c(0.25, 0.5, 0.75, 1, 1.5, 2, 2.5)*ggpredict(mod_full, "slope [mean]", condition = c(here_speed = 6, project_year = 2005))["predicted"]%>%pull, Inf)
    ...
    geom_contour(
      breaks = 
        ggpredict(
          mod_full,
          "slope [mean]", condition = c(here_speed = 6, project_year = 2005)
        )["predicted"],
      color = "black",
      linetype = "solid",
      size = 1
...
fig_sitesdist <- (
  ...
  map_df(mods, predict_cost_interaction, var1 = "n_worksites", var2 = "tot_dist", lims1 = c(0, 6), lims2 = c(0, 10000), by1 = 0.1, by2 = 100, .id = "model") %>%
    ...
    geom_contour_filled(
      breaks = c(0, c(0.25, 0.5, 0.75, 1, 1.5, 2, 2.5)*ggpredict(mod_full, "n_worksites [1]", condition = c(here_speed = 6, project_year = 2005))["predicted"]%>%pull, Inf)
    ) +
    scale_fill_brewer(
      name = str_wrap("Predicted cost per culvert, relative to mean", 30),
      direction = -1,
    ...
    geom_contour(
      breaks = 
        ggpredict(
          mod_full,
          "n_worksites [1]",
          condition = c("tot_dist" = 0, here_speed = 6, project_year = 2005),
        )["predicted"],
      size = 1, color = "black",
      linetype = "solid"
    ) +
...
cowplot::save_plot(
    ...
    filename = here("output/figs/fig_contours.png"), base_width = 6.5, base_height = 7.5
